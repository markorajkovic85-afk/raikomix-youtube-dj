          ...
          <section className="flex-1 flex flex-col p-4 items-center justify-center overflow-auto">
            <div className="flex flex-col lg:flex-row gap-6 items-center">
              <Deck ref={deckARef} id="A" color="#D0BCFF" eq={deckAEq} onStateUpdate={s => handleDeckStateUpdate('A', s)} onPlayerReady={p => setMasterPlayerA(p)} />
               deckAEffect={deckAState?.effect || 'NONE'} onDeckAEffectChange={(effect) => deckARef.current?.updateState({effect})} deckAEffectWet={deckAState?.effectWet || 0.5} onDeckAEffectWetChange={(wet) => deckARef.current?.updateState({effectWet: wet})} deckBEffect={deckBState?.effect || 'NONE'} onDeckBEffectChange={(effect) => deckBRef.current?.updateState({effect})} deckBEffectWet={deckBState?.effectWet || 0.5} onDeckBEffectWetChange={(wet) => deckBRef.current?.updateState({effectWet: wet})} crossfader={crossfader} onCrossfaderChange={setCrossfader} crossfaderCurve={xFaderCurve} onCurveChange={setXFaderCurve} masterVolume={masterVolume} onMasterVolumeChange={setMasterVolume} deckAVolume={deckAVolume} onDeckAVolumeChange={setDeckAVolume} deckBVolume={deckBVolume} onDeckBVolumeChange={setDeckBVolume} deckAPlaying={deckAState?.playing || false} deckBPlaying={deckBState?.playing || false} deckAEq={deckAEq} deckBEq={deckBEq} onDeckAEqChange={(k, v) => setDeckAEq(p => ({...p, [k]: v}))} onDeckBEqChange={(k, v) => setDeckBEq(p => ({...p, [k]: v}))} />
